...
.. highlight:: ini
   ...
   Each middleware is defined by its ``name``, and optionally a ``position``. If position is not
   specified, the middleware will be added at the end of the middleware stack. For example::

   [middleware]
   midleware1_name=plugin.middleware.Plugin2Middleware
   middleware1_position = insert_after:django.middleware.common.CommonMiddleware
   middleware2_name = plugin.middleware.Plugin1Middleware
